...
Strategies for tasks that exceed context window limits. Not mutually exclusive — combine based on task characteristics.

## Strategy Selection

| Task Characteristic | Best Strategy | Why |
|---|---|---|
| Extensive back-and-forth | Compaction | Maintains flow without losing thread |
| Iterative development with milestones | Structured notes | Tracks progress, survives compaction |
...
| Very long tasks (hours) | Combine all | Each addresses a different constraint |
...
3. **Test on complex agent traces**, not simple conversations. Simple exchanges compress trivially; hard cases reveal prompt weaknesses.

### Tool Result Clearing (Lightest Touch)

Before full compaction, clear old tool call results. Once a tool was called deep in history, only the conclusion drawn from it matters.

```
Before: [tool_call: grep "auth" → 200 lines] → [analysis: "auth handled in middleware/auth.ts"]
After:  [tool_call: grep "auth" → <cleared>] → [analysis: "auth handled in middleware/auth.ts"]
```

### Trigger Heuristics

- **Token threshold**: ~80% of window limit
- **Turn count**: After N tool-heavy turns, evaluate whether clearing results suffices
- **Signal decay**: Agent repeats searches or forgets conclusions → compact immediately
- **Manual**: User or orchestrator explicitly requests compaction

### Warning

Overly aggressive compaction loses subtle context whose importance emerges later. A design constraint mentioned in turn 3 might become critical in turn 40. Better to over-include. You can compact again; you cannot recover lost context.

## Structured Note-Taking

### Patterns

- **Todo list**: Progress, completed items, blockers. Maps to task management tools (TaskCreate, TaskUpdate).
- **Scratchpad (NOTES.md)**: Architectural decisions, key findings, dependency relationships. Free-form but scannable.
...
- Critical decisions made early affect later work (always retrievable)
...
- Each sub-agent starts with **clean context** (only what it needs for its task)
- Sub-agents may use 10K+ tokens internally for exploration, reasoning, tool calls
- Returns **condensed summary** (1-2K tokens) — the distilled output
...
The constraint on summary length forces distillation. Without it, sub-agents return full transcripts that defeat the purpose.

## Compressed Index Pattern

See [SKILL.md](../SKILL.md) §Law 2 for the canonical format and example. See [passive-context-patterns.md](passive-context-patterns.md) for the Vercel implementation that achieved 100% pass rate.

Key implementation rules:
1. Pipe-delimited format for maximum compression
2. Group files by domain or feature
3. Include root path so the agent resolves reads without guessing
4. Add retrieval-led reasoning instruction at the top
5. Update the index when files are added or removed

When NOT to use: small projects (full docs fit in context), rapidly changing file structures (index goes stale), tasks needing every file simultaneously (just inline them).

## Combining Strategies

For tasks spanning hours or multiple sessions, layer all strategies:
...
4. **Compaction** as periodic maintenance that resets conversation while preserving thread

**Sequencing**: Index set at session start. Notes accumulate throughout. Sub-agents spin up for intensive phases. Compaction fires at limits, using notes as anchor to validate the summary.
