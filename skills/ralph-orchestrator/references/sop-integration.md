...
2. **Execution Phase**: Agent Teams executing the plan
...
sequenceDiagram
    ...
    O->>C: TeamCreate + spawn teammates
...
### Task Generation → Execution (Agent Teams)
...
# Orchestrator launches Agent Teams via TeamCreate + spawn teammates
# Teammates read:
...
**Note:** Agent Teams operates in **SOP mode only**:
...
**Action**: Circuit breaker stops task cycle, update plan, spawn new teammates
...
Resume: spawn new teammates for remaining PENDING tasks
```
...
- Agent Teams: ~1-1.5 hours human time + ~3 hours autonomous
...
# Agent Teams execution continues autonomously
...
- You MUST update specs and re-spawn teammates if information is truly missing
